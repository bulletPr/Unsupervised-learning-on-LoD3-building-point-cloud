...
def main():
    ...
    parser.add_argument('--block_size', '-b', help='Block size', type=float, default=1.0)
    parser.add_argument('--grid_size', '-g', help='Grid size', type=float, default=0.02)
                    ...
                    for block_split_idx in range(block_split_num):
                        ...
                        if ((idx + 1) % batch_size == 0) or \
                            ...
                            if args.save_ply:
                                ...
                                filepath_label_ply = os.path.join(output_dir, 'ply_label',
                                                                  dataset + '_label_%s_%d' % (offset_name, idx_h5))
                                data_utils.save_ply_property_batch(
                                    data[0:item_num, :, 0:3],
                                    label_seg[0:item_num, ...],
                                    filepath_label_ply, data_num[0:item_num, ...], 9)

                                filepath_rgb_ply = os.path.join(output_dir, 'ply_rgb',
