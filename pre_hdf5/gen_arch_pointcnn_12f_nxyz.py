...
def main():
    ...
    batch_size = 2048
    data = np.zeros((batch_size, max_point_num, 12))
    ...
    for folder in folders:
        ...
        for dataset_idx, dataset in enumerate(datasets):
            ...
            xyz = xyz - xyz_center
            rgb = all_data[:,3:6] / 255.0
            hsv = color.rgb2hsv(rgb)
            normal = all_data[:,7:10]
            offsets = [('zero', 0.0), ('half', args.block_size / 2)]
            for offset_name, offset in offsets:
                idx_h5 = 0
                idx = 0

                print('{}-Computing block id of {} points...'.format(datetime.now(), all_data.shape[0]))
                xyz_min = np.amin(xyz, axis=0, keepdims=True) - offset
                xyz_max = np.amax(xyz, axis=0, keepdims=True)
                scene_max_x = max(xyz_max[:,0])
                scene_max_y = max(xyz_max[:,1])
                scene_max_y = max(xyz_max[:,2])
                block_size = (args.block_size, args.block_size, 2 * (xyz_max[0, -1] - xyz_min[0, -1]))
                xyz_blocks = np.floor((xyz - xyz_min) / block_size).astype(np.int)

                print('{}-Collecting points belong to each block...'.format(datetime.now(), all_data.shape[0]))
                ...
                for block_idx in range(idx_last_non_empty_block + 1):
                    ...
                    block_points = xyz[point_indices]
                    block_points[..., 0]/=max_room_x
                    block_points[..., 1]/=max_room_x
                    block_points[..., 2]/=max_room_x
                    Nx, Ny, Nz = np.split(block_points, (1, 2), axis=-1)
                    block_xzyrgbnormal = np.concatenate([xyz[point_indices], Nx, Nz, Ny, hsv[point_indices], normal[point_indices]], axis=-1)
                    ...
                    for block_split_idx in range(block_split_num):
                        ...
                        data[idx_in_batch, 0:point_num, ...] = block_xzyrgbnormal[start:end, :]
